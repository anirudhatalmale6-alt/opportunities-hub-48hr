...
function opphub_activate() {
    ...
    // Create the Corporate Services page if it doesn't exist
    $corp_page = get_page_by_path('corporate-services');
    if (!$corp_page) {
        wp_insert_post([
            'post_title'   => 'Corporate Services',
            'post_name'    => 'corporate-services',
            'post_status'  => 'publish',
            'post_type'    => 'page',
            'post_content' => '<!-- This page uses the Corporate Services template. Content is generated by the plugin. -->',
            'page_template'=> '',
            'meta_input'   => ['_wp_page_template' => 'opphub-corporate.php'],
        ]);
    }
...
function opphub_page_template($template) {
    ...
    if (is_page('corporate-services') || (is_page() && get_page_template_slug() === 'opphub-corporate.php')) {
        $plugin_template = OPP_HUB_PATH . 'templates/corporate-services.php';
        if (file_exists($plugin_template)) {
            return $plugin_template;
        }
    }
...
function opphub_add_page_template($templates) {
    ...
    $templates['opphub-corporate.php'] = 'Corporate Services';
    return $templates;
}
...
function opphub_no_cache_headers() {
    if (is_page('funding-hub') || is_page('testimonials') || is_page('corporate-services') || (is_page() && in_array(get_page_template_slug(), ['opphub-landing.php', 'opphub-testimonials.php', 'opphub-corporate.php']))) {
        nocache_headers();
...
function opphub_enqueue_assets() {
    ...
    $is_corporate = is_page('corporate-services') || (is_page() && get_page_template_slug() === 'opphub-corporate.php');

    if ($is_hub || $is_testimonials || $is_corporate) {
